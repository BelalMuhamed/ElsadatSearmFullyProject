...
import {
  AccountDetailsDto,
  AccountDetailsDtoReq,
  AccountDto,
  CreateAccountDto,
  DisAndMerchAccountDto,
  FilterationAccountsDto,
  TreeAccountDto
} from '../models/TreeAccountDto';
...
export class TreeAccountsService {
  ...
  /** Account code is auto-generated by the server — it is intentionally NOT in the payload. */
  addAccount(account: CreateAccountDto): Observable<Result<string>> {
    return this.http.post<Result<string>>(`${this.apiUrl}TreeAccounts`, account);
  }

  /** Account code is read-only — the server ignores it on update. */
  ...
  getByAccountId(id: number): Observable<Result<AccountDto>> {
    return this.http.get<Result<AccountDto>>(`${this.apiUrl}TreeAccounts/${id}`);
  }

  getAccountJournalEntryDetails(req: AccountDetailsDtoReq): Observable<Result<AccountDetailsDto>> {
    const params: any = {
      accountId: req.accountId,
      page: req.page,
      pageSize: req.pageSize
    };
    if (req.entryId) params.entryId = req.entryId;
    if (req.entryDate) params.entryDate = req.entryDate;

    return this.http.get<Result<AccountDetailsDto>>(`${this.apiUrl}JournalEntry`, { params });
  }
}
